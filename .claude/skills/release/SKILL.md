---
name: release
description: "Cut a release of @ironarachne/word-generator — bump the version through a pull request, tag main, and verify the tag-driven publish to npm. Use when the user asks to cut, publish, or ship a release, bump the version, or mentions a version number to release."
---

# Releasing @ironarachne/word-generator
...
`2.5.0`. If none is given, ask which — do not guess from the diff.

Reach for `major` whenever the change alters the word a given pattern and seed
produce, even with no signature change — including a bump of the
`@ironarachne/rng` major version. See `CODE_STYLE.md` for why that is the
contract that matters in this package.
...
```bash
npm view @ironarachne/word-generator versions --json
```
...
for i in $(seq 1 40); do
  v=$(curl -s https://registry.npmjs.org/@ironarachne%2Fword-generator | python3 -c "import json,sys;print(json.load(sys.stdin).get('dist-tags',{}).get('latest'))")
...
```bash
curl -s https://registry.npmjs.org/@ironarachne%2Fword-generator/<X.Y.Z> \
